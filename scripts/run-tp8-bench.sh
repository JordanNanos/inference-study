...
for TP in 8 4; do
    for NUM_STEPS in 1 2 4 8; do
        #for NUM_SEQS in 512; do
            #for NUM_TOKS in 1024; do
                #mkdir -p $DIR/tp$TP-nseqs$NUM_SEQS-nsteps$NUM_STEPS
                mkdir -p $DIR/tp$TP
                #if using docker
                ...
                for con in 4 8 16 32 64 128; do for input in 256; do for output in 128; do python -u vllm/benchmarks/benchmark_serving.py --disable-tqdm --model $MODEL --base_url $HOST --ignore-eos --max-concurrency $con --percentile-metrics ttft,tpot,itl,e2el --metric-percentiles 25,50,75,99 --random-range-ratio 0.8 --dataset-name random --request-rate inf --random-input-len ${input} --random-output-len ${output} --num-prompts 256 > $DIR/tp$TP/${con}conn.log; done; done; done
